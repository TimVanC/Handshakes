#!/usr/bin/env node
/** Safely append one generated MLB batch after screening every local schedule surface. */
import fs from "node:fs";

const [jsonPath, fragmentPath] = process.argv.slice(2);
if (!jsonPath || !fragmentPath) throw new Error("usage: node pipeline/import-mlb-batch.mjs batch.json batch.tsfrag");

const batch = JSON.parse(fs.readFileSync(jsonPath, "utf8"));
const fragment = fs.readFileSync(fragmentPath, "utf8").trimEnd();
const puzzlePath = "src/data/mlb/puzzles.ts";
const rosterPath = "src/data/mlb/roster.ts";
...
const activeEnd = /\r?\n\];\r?\n\r?\n\/\*\*\r?\n \* Benched/;
if (!activeEnd.test(puzzleSource)) throw new Error("active MLB puzzle insertion marker not found");
