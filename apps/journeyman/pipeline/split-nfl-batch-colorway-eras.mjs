#!/usr/bin/env node
/** Split colorway records touched by a batch at NFL renderer construction boundaries. */
import fs from "node:fs";

const batchPath = process.argv[2];
if (!batchPath) throw new Error("usage: node pipeline/split-nfl-batch-colorway-eras.mjs batch.json");
const batch = JSON.parse(fs.readFileSync(batchPath, "utf8"));
const colorwayPath = "../../packages/jerseys/data/nfl/colorways.json";
...
const boundaries = [1978, 1996, 2012];
const styleFor = (year) => year < 1978 ? "classic" : year < 1996 ? "stripes" : year < 2012 ? "nineties" : "modern";
let splitCount = 0;
for (const [franchise, eras] of Object.entries(db.franchises)) {
  ...
  for (const era of eras) {
    ...
    for (let index = 0; index < starts.length; index++) {
      ...
      replacements.push({ ...era, key: index === 0 ? era.key : `${franchise}_${start}`, years: [start, ends[index]], eraStyle: styleFor(start) });
    }
  }
...
console.log(`Split ${splitCount} touched NFL colorway renderer boundaries.`);
